...
namespace PPDConfiguration
{
    ...
    public class SettingReader
    {
        ...
        /// <param name="defaultValue">既定値</param>
        /// <returns></returns>
        public T ReadEnum<T>(string key, T defaultValue) where T : struct, IConvertible
        {
            var getValue = ReadInt(key, defaultValue.ToInt32(CultureInfo.InvariantCulture));
            foreach (T v in Enum.GetValues(typeof(T)))
            {
                if (v.ToInt32(CultureInfo.InvariantCulture) == getValue)
                {
                    return v;
                }
            }
